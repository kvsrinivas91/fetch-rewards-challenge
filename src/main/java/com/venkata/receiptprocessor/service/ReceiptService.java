...
public class ReceiptService {
    ...
    public boolean calculateUniqueProductsPoints(List<Item> items){
        HashSet<Item> hashSet = new HashSet<>();


        for(int i=0;i<items.size();i++){

            if(hashSet.contains(items.get(i))){
                return false;
            }
            hashSet.add(items.get(i));
        }
    ...
    public int calculatePoints(Receipt receipt){
        ...
        if(calculateUniqueProductsPoints(receipt.getItems())==true){
            points+=20;
        }
